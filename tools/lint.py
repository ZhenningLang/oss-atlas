...
# Pilot phase: validate the block + card ONLY when a page carries one (missing
# is silent). Phase 2 flips missing -> error once all pages are backfilled.
GRADES = {"A", "B", "C", "D", "E", "?"}
...
def check_health_block(path: Path, text: str, base: str, zh: bool, category_dir: Path, rep: Report) -> None:
    ...
    if not re.search(r"(?m)^health:\s*$", fmtext):
        return  # no block yet — backfilled in Phase 1; required in Phase 2
