...
| [guizang-ppt-skill](guizang-ppt.zh.md) | ✅ | 只需要 deck 生成这个单一 Skill 时，选 guizang-ppt-skill。 | 单一用途的 deck 生成 Skill；Open Design 把 deck 生成作为众多产物类型之一，并自带运行时/导出。 |
| [guizang-social-card-skill](guizang-social-card.zh.md) | ✅ | 产物明确是社交卡片时，选 guizang-social-card-skill。 | 专注社交卡片的 Skill；Open Design 在一个打包应用里覆盖卡片/图像等多种产物类型。 |
