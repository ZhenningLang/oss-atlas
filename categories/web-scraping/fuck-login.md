...
|---|---|---|---|
| Playwright / Selenium | 未收录 | Choose Playwright or Selenium when you need a real browser to handle JS and modern auth. | Drive a real browser to log in (handles JS, modern auth) instead of replaying raw HTTP; heavier but actually works on today's sites. |
