...
| 替代品 | 是否收录 | 取舍 |
|---|---|---|
| [python-readability](python-readability.zh.md) | ✅ | 基于 lxml 的同一 arc90 血统 Python 移植；按语言选（Python 管线）——启发式与输出和 JS 引擎有别。 |
| [dragnet](dragnet.zh.md) | ✅ | ML 模型做正文抽取（Python）；在某些页面上可胜出，但更重、依赖老化、维护较少。 |
| [boilerpipe](boilerpipe.zh.md) | ✅ | 经典 Java 样板移除算法；思想成熟，但仓库实际已废弃（最后 push 2018）。 |
| trafilatura | 未收录 | Python 抽取库，基准结果强、带元数据和爬取支持；常是现代 Python 默认——语言不同、范围更广。 |
| Mercury / Postlight Parser | 未收录 | Node 文章解析器，也会抓页面；历史上流行但维护起伏不定。 |
