...
- **治理与 bus factor——单一开发者、star 与成熟度失配 ⇒ 强红旗。** 一个 `User` 持有的仓库（`@thedotmack`）坐在你每次会话的关键路径上，是 bus-factor 为一的依赖。约 84.8k star 这个数字对一个年轻的单人 hook 工具极不成比例，与其成熟度*不*相符——请把热度与检验程度解耦，别当成采用证据。[未验证]
