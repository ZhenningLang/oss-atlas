...
tags: [video-production, claude-video, app]
language: Python
license: NOASSERTION
...
health:
  schema: 1
  computed_at: 2026-07-16T08:14:50Z
  overall: B
  overall_score: 2.6
  scored_axes: 5
  capped: false
...
You're evaluating a task in the `video-production` area and want a real repository in the oss-atlas shortlist rather than an untracked name from a backlog. Reach for claude-video when the upstream description matches the job, when its license and maintenance profile are acceptable after verification, and when adopting a public project is preferable to writing a local one-off.
...
- **Repository shape:** `bradautomates/claude-video`; this first-pass page has not exhaustively read every dependency manifest.
- **Default branch snapshot:** last pushed `2026-07-01T01:26:49Z`; archived `false`.
...
- **Maintenance snapshot (2026-07-16):** GitHub reports `archived=false` and `pushed_at=2026-07-01T01:26:49Z`.
- **Adoption snapshot:** ~8,688 GitHub stars as of 2026-07; this is a noisy signal and low-star projects are still included when the repository is real and relevant.
