...
| [Playwright](playwright.zh.md) / Puppeteer | 部分已收录 | 需要成熟、代码优先的 Node 自动化库时，选 Playwright 或 Puppeteer。 | 成熟的 Node.js 自动化库，跨浏览器、API 丰富——但你要以代码方式 import 并自己写/维护选择器；Agent Browser 是 CLI，带 LLM 友好的 refs 且无每命令启动开销。 |
