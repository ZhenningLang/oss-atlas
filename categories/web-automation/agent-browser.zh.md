...
| Playwright / Puppeteer | 未收录 | 需要成熟、代码优先的 Node 自动化库时，选 Playwright 或 Puppeteer。 | 成熟的 Node.js 自动化库，跨浏览器、API 丰富——但你要以代码方式 import 并自己写/维护选择器；Agent Browser 是 CLI，带 LLM 友好的 refs 且无每命令启动开销。 |
| browser-use | 未收录 | 需要自带 LLM 循环的 Python 浏览器 agent 时，选 browser-use。 | Python，视觉+DOM 浏览器 agent，自带 LLM 循环。Agent Browser 是更底层的 Rust CLI 原语（agent 循环你自己提供），shell 调用更快、MCP 原生。 |
...
- **响应速度**：Grade A——中位首次响应时间 118.9 小时，基于 15 个 qualifying issues/PRs。
