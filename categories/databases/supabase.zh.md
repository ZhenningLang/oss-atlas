...
你正在构建 Web 或移动应用，需要一个能处理身份认证、数据库、实时订阅和文件存储的后端，而无需拼凑多个服务。你想要 PostgreSQL 的强大和熟悉度（包括 `pgvector` 和 `postgis` 等扩展），但不想分别管理数据库、API 层和身份认证系统。你选择 Supabase，创建项目，即可获得自动生成的 REST API、开箱即用的 OAuth 身份认证系统，以及从 Postgres 表直接获取的实时 WebSocket 订阅。对于 AI 功能，你可以用 `pgvector` 存储和查询向量嵌入，无需添加单独的向量数据库。你可以完全自托管整个栈，也可以使用托管云服务并享有慷慨的免费层。

## 何时不用

- **重度分析/OLAP 负载**——Supabase 基于 PostgreSQL，针对 OLTP 优化。对于大规模数据仓库或复杂分析，请使用专用 OLAP 解决方案（BigQuery、ClickHouse、Snowflake）。
- **多数据库的微服务架构**——Supabase 围绕每个项目单个 Postgres 实例设计。如果你的架构需要多个独立数据库和独立生命周期，平台模式可能不适合。
- **不熟悉 PostgreSQL**——虽然 Supabase 抽象了大量数据库层，但你仍需为复杂查询、迁移和 RLS 策略编写 SQL。如果你的团队完全回避 SQL，学习曲线确实存在。
- **极端多区域延迟需求**——Supabase 支持只读副本，但主写入节点位于单一区域。如果你的应用需要全球亚 10ms 写入，可能需要不同架构。
- **非 Postgres 数据库需求**——Supabase 与 PostgreSQL 深度绑定。如果你需要 MongoDB、Cassandra 或图数据库作为主存储，Supabase 不是正确选择。
...
- **维护**：非常活跃——截至 2026-07 每日推送，v2 平台成熟，核心团队响应迅速（1,086 个开放 issue）。[推断]
- **治理**：由 `supabase` 组织所有，拥有专门的核心团队和清晰的产品路线图。公司以 Apache-2.0 许可为核心栈，具备强大的开源文化。bus factor 合理。[推断]
- **背书**：Supabase Inc. 是获得大量投资的创业公司，拥有成熟的收入模式（托管云）。开源核心与商业产品战略对齐。[未验证]
- **采用**：采用度强劲，105.0k star，2019 年创建（7 年记录）。被初创公司和企业在生产环境中广泛使用。Firebase 替代定位带来持续需求。[推断]
- **风险旗标**：Apache-2.0 非常宽松。风险投资模式意味着某些功能可能仅限云端（如某些高级分析或企业功能），但核心数据库、身份认证和存储栈保持完全开源。需关注是否随时间出现 open-core 阉割。[推断]
