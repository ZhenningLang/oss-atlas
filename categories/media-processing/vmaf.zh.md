...
| [FFmpeg](ffmpeg.zh.md) | ✅ | 需要在媒体管线里把 `libvmaf` 作为滤镜运行时，选 FFmpeg。 | 把 `libvmaf` 作为滤镜集成——对多数用户而言这才是你在管线里真正跑 VMAF 的方式；FFmpeg 是宿主，VMAF 是其中的指标引擎。 |
...
- **响应速度**：Grade B——中位首次响应时间 41.2 小时，基于 4 个 qualifying issues/PRs。
