...
health:
  schema: 1
  computed_at: 2026-07-03T03:19:43Z
  overall: B
  overall_score: 3.4
  scored_axes: 5
  capped: false
...
| GStreamer | 未收录 | Use GStreamer for real-time, persistent, app-embedded pipelines; use MLT for offline timeline-based editing/compositing. | Pipeline/element graph framework for live/streaming and app-embedded media; heavier programming model but more composable for real-time apps. |
| HandBrake | 未收录 | Use HandBrake for end-user preset-driven transcoding; use MLT for programmatic timeline editing. | Preset-driven GUI and CLI for ripping/transcoding to modern MP4/MKV; end-user app, not a library, far narrower than raw FFmpeg. |
| MoviePy | 未收录 | Use MoviePy for friendly Python API batch video editing; use MLT for C++ timeline framework with editorial precision. | Friendly Python API for programmatic video editing — cutting, compositing, text, effects — but batch-only and slower than raw FFmpeg for large files. |
| PyAV | 未收录 | Use PyAV for Pythonic FFmpeg bindings; use MLT for timeline model and editorial semantics. | Pythonic bindings to FFmpeg's libav*; gives you codec-level control in Python, but no timeline or NLE abstractions. |
