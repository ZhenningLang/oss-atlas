...
|---|---|---|---|
| Hand-rolled regex / manual parsing | 未收录 | Use this page for its stated niche; choose Hand-rolled regex / manual parsing when you need no dependency, but you reimplement the tag grammar, variant/media structure, and serialization. | No dependency, but you reimplement the tag grammar, variant/media structure, and serialization — and HLS has many edge cases this library already handles. |
| hls.js / other-language parsers | 未收录 | Use this page for its stated niche; choose hls.js / other-language parsers when you need mature HLS parsers in other runtimes (JS, Go). | Mature HLS parsers in other runtimes (JS, Go); same job, wrong language if you're in Python. |
| [FFmpeg](ffmpeg.md) | ✅ | Use this page for its stated niche; choose FFmpeg when you need reads/writes HLS as part of (de)muxing media, but it's an opaque media engine. | Reads/writes HLS as part of (de)muxing media, but it's an opaque media engine — not a manipulable manifest *object model* you can introspect and rewrite tag-by-tag. |
| streamlink / yt-dlp (as consumers) | 未收录 | Use this page for its stated niche; choose streamlink / yt-dlp (as consumers) when you need full stream-download tools that parse HLS internally. | Full stream-download tools that parse HLS internally; use them to *grab* a stream, not as a library to *manipulate* manifests in your own code. [未验证] |
