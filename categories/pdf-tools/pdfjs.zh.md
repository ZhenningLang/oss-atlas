...
| 替代品 | 是否收录 | 我们的评价 | 取舍 |
|---|---|---|---|
| pdf-lib | 未收录 | 当前页用于它的主场景；如果更看重“**创建和修改** PDF 的 JS 库（表单、合并、绘制）”，再选 pdf-lib。 | **创建和修改** PDF 的 JS 库（表单、合并、绘制）——正是 PDF.js 不覆盖的写一侧；它不是渲染器/阅读器。 |
| jsPDF | 未收录 | 当前页用于它的主场景；如果更看重“客户端用 JS **生成** PDF”，再选 jsPDF。 | 客户端用 JS **生成** PDF；是互补而非替代——它造文档，PDF.js 显示文档。 |
| PyMuPDF / pdfplumber | 未收录 | 当前页用于它的主场景；如果更看重“Python 库，服务端快速渲染 + 文本/表格抽取”，再选 PyMuPDF / pdfplumber。 | Python 库，服务端快速渲染 + 文本/表格抽取；后端批量作业更合适，但不是浏览器阅读器。 |
| [Docling](../document-parsing/docling.zh.md) | 未收录 | 当前页用于它的主场景；如果更看重“版面感知的文档解析器，产出结构化结果（阅读顺序、表格）供 AI/RAG”，再选 Docling。 | 版面感知的文档解析器，产出结构化结果（阅读顺序、表格）供 AI/RAG；目标不同——要的是语义结构，不是像素级忠实显示。 |
| 原生 `<embed>` / 浏览器 PDF 插件 | 未收录 | 当前页用于它的主场景；如果更看重“零依赖、浏览器自带，但跨浏览器表现不一致、不可控，也给不了你程序化的文本/渲染访问”，再选 原生 <embed> / 浏览器 PDF 插件。 | 零依赖、浏览器自带，但跨浏览器表现不一致、不可控，也给不了你程序化的文本/渲染访问。 |
