...
| [Superpowers](../../../agent-dev-methodology/superpowers.md) | ✅ | Choose Superpowers when you need the full brainstorm→plan→TDD→verify lifecycle. | Full brainstorm→plan→TDD→verify SDLC methodology pack; its `verification-before-completion` / `systematic-debugging` skills overlap PUA's Red Lines but ship a whole lifecycle, where PUA is just the persistence/anti-quit layer with a persona skin. |
