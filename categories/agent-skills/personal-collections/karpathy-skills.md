...
| [Superpowers](../../agent-dev-methodology/superpowers.md) | ✅ | Pick Superpowers when you need a full SDLC methodology graph rather than a few lightweight behavioral principles. | A full SDLC methodology library (brainstorm→plan→TDD→verify) with many composable skills and per-harness manifests. Karpathy-skills covers a similar "make the agent disciplined" goal in four lines instead of a skill graph — far lighter, far less prescriptive. |
