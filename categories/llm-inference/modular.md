...
| [TensorRT-LLM](tensorrt-llm.md) | ✅ | Use this page for its stated niche; choose TensorRT-LLM when you need NVIDIA's own engine, top-tier performance on NVIDIA hardware. | NVIDIA's own engine, top-tier performance on NVIDIA hardware; deeply NVIDIA-locked, heavier build/engine-compile workflow, no cross-vendor abstraction. |
| [Ray Serve](ray-serve.md) | ✅ | Use this page for its stated niche; choose Ray Serve when you need general Python model-serving/orchestration framework for scaling and composing services. | General Python model-serving/orchestration framework for scaling and composing services; not a hand-tuned single-model inference engine — different layer. |
...
| [SGLang](sglang.md) | ✅ | Use this page for its stated niche; choose SGLang when you need a Python-native open-source serving engine with structured generation and RadixAttention prefix caching. | High-throughput serving engine with RadixAttention prefix caching and structured-generation optimizations; newer, smaller ecosystem than vLLM, not a cross-vendor compiler platform. |
