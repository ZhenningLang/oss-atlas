...
| 替代品 | 是否收录 | 取舍 |
|---|---|---|
| VSCodeVim | 未收录 | VS Code 的 Vim 模拟；同样的点子换个宿主——按你实际用哪个 IDE 来选，而非按插件。 |
| vscode-neovim | 未收录 | 在 VS Code 内嵌入一个*真* Neovim 实例，保真更高＋可用你的 Neovim 配置；更重且仅限 VS Code，JetBrains 侧没有这种深度的对应物。 |
| 真 Vim / Neovim | 未收录 | 货真价实，拥有完整插件生态与 Lua/LSP；但你会失去 JetBrains 的集成重构/调试器/索引。 |
| JetBrains 自带键位 | 未收录 | 没有模拟层、完全受支持；但没有 Vim 模式/motion——对 Vim 用户而言失去意义。 |
