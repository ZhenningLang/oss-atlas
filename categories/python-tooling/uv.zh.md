...
你是一位 Python 开发者，厌倦了等待 `pip install` 解析依赖，或疲于在多个工具间切换——pip 安装、pip-tools 锁定、pipx 管理 CLI 工具、pyenv 管理 Python 版本、poetry 管理项目。你想要一个工具，它比 pip 快 10–100 倍安装包、管理 Python 版本、运行带内联依赖元数据的脚本，并产出可入 Git 的通用锁文件。你正在启动新 Python 项目或现代化现有项目，想要最快、最可靠的打包体验。

## 何时不用

- **如果你需要成熟、久经考验的生态**——uv 相对较新（2023 年创建）。虽然采用迅速，但某些依赖解析的边缘情况或平台特定构建可能仍不如 pip 或 poetry 平滑。
- **如果你依赖 poetry 的特有功能**——Poetry 的 `pyproject.toml` extras、插件和构建后端生态并非完全兼容。迁移现有 poetry 项目可能需要手动调整。
- **如果你需要 conda-forge 或科学计算二进制包**——uv 不能替代 Conda/Mamba 用于需要预编译二进制分发的科学栈。它是 pip 的替代，不是 Conda 的替代。
- **如果你在使用没有 Rust 工具链的冷门平台**——uv 为常见平台提供预编译二进制，但小众架构可能需要从源码构建。
- **如果你的团队还没准备好改变工作流**——uv 引入了新的命令（`uv pip`、`uv run`、`uv lock`），与标准 pip/virtualenv 工作流不同。对于稳定的老项目，学习曲线可能不值得速度收益。
