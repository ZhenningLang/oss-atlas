...
当你要交付一个需要离线/端侧 LLM 推理的**移动端或跨平台 App**(尤其是 Android,其 Kotlin 支持稳定且为一等公民),目的是保护隐私、实现零网络运行,或规避按调用计费的云端成本——并且你的模型选型是 **Gemma**(或其他已发布为 `.litertlm` 格式的模型)时,使用它。

当你希望用一套运行时横跨 Android/iOS/桌面/Raspberry Pi,搭配 CPU 加上可选的 GPU/NPU 加速,偏好 Google 维护的工具链而非社区拼装,并且能接受小模型的质量上限(短小、结构化任务:分类、抽取、摘要、端侧助手)时,它是强契合方案。第三方基准测试显示,它在 iPhone 上特别针对 Gemma 级模型时表现具竞争力乃至最快,因此以 Gemma 为中心、对延迟敏感的端侧 App 正是它的甜点区。
