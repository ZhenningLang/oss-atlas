...
|---|---|---|---|
| llama.cpp | 未收录 | Choose llama.cpp when you need broad GGUF model/quantization support and ubiquitous reach. | Far broader model/quantization support (GGUF ecosystem) and ubiquitous reach, but more cross-platform build complexity and no single Google-blessed mobile SDK — you assemble more glue. |
| MLX / mlx-lm (Apple) | 未收录 | Choose MLX when Apple-silicon speed and clean Swift/Python ergonomics matter most. | Faster than LiteRT-LM on many non-Gemma models with clean Swift/Python ergonomics on Apple silicon, but Apple-only — can't be your cross-platform answer. |
