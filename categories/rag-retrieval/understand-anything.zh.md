...
language: TypeScript
license: MIT
maturity: "v2.7.3 (2026-05), active (2026-06); ~68.8k stars claimed but unverified/suspicious for a young repo — flag, don't trust"
...
- **可疑的人气 / 信任信号。** 一个仅约 603 次提交的仓库声称约 68.8k star，按其历史看高得不合常理，不应当作社会证明——见存疑。别*因为* star 数而选它。
...
| Sourcegraph / SCIP | 未收录 | 工业级精确代码智能（跨仓、language server、规模化）；基础设施更重，不是 agent 插件形态的即插件。Understand-Anything 更轻、有 LLM 增强，但未经证明。 |
...
- **信任信号——可疑的人气，硬标志。** 一个约 603 次提交的仓库声称约 68.8k star，按其历史看高得不合常理[未验证]——很可能被灌水或是数据异常。**别*因为* star 数而选它**；star 与历史的失配本身就是警告，而非社会证明。
...
- [未验证] **约 68.8k 的 star 数可疑且未经核实。** 一个仅约 603 次提交、7 个 release、首发历史这么短的仓库，正常不会积累约 68.8k star；该数字对其年龄/活跃度而言不合常理，可能被灌水或是数据异常。**不要**当作社会证明或质量信号。
