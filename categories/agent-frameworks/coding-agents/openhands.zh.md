...
| 替代品 | 是否收录 | 我们的评价 | 取舍 |
|---|---|---|---|
| [aider](aider.zh.md) | ✅ | 当你需要本分类里已经收录、约束更明确的方案时，先用它和 OpenHands 对照。 | OpenHands 是从 intake backlog 新增的首版页面；现有页面的“不用场景”如果更贴近任务，应优先按现有页面选择。 |
...
| [Cline](cline.zh.md) | ✅ | 当你需要本分类里已经收录、约束更明确的方案时，先用它和 OpenHands 对照。 | OpenHands 是从 intake backlog 新增的首版页面；现有页面的“不用场景”如果更贴近任务，应优先按现有页面选择。 |
