...
- **采用度。** star（约 9.6k）和 fork（约 912）对一个一个月大的仓库而言高得反常——很可能是发布/曝光高峰（Microsoft + 当下热门的"skills"叙事），**不是**生产采用的证据。视为热度信号，而非社会证明。[未验证]
- **风险标记。** MIT（干净）。主要标记：全新（无寿命）、研究级 v0.1.0 且失败模式/成本上界无文档，以及读起来更像发布热度而非使用度的采用指标。[推断]

## 存疑（未验证）

- [未验证] 截至 2026-06，一个约 1 个月大的仓库有约 9.6k star / 约 912 fork——以这个年龄看这些计数高得不合常理，几乎必然反映发布高峰而非持续采用；请高度存疑。
