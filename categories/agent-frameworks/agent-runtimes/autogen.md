...
| [Hermes Agent](hermes-agent.md) | ✅ | When you need the established in-index option for this category, compare it against AutoGen before switching. | AutoGen is newly indexed from the intake backlog; use the existing page when its documented constraints match better, and choose AutoGen only after verifying the repo-specific caveats below. |
