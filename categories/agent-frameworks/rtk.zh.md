...
你是一位开发者或团队，正在使用 AI 编码智能体（Claude Code、Codex、Open Interpreter），而 LLM API 账单因每个 `git diff`、`ls -la`、`find` 和 `cat` 输出都被原样丢进上下文窗口而不断攀升。你想要一个透明代理，坐在 shell 与智能体之间，自动压缩重复输出、截断冗长列表、总结大型 diff——无需你手动通过 `| head` 管道命令。你需要它快到让你感知不到开销，并希望它是零运行时依赖的单一静态二进制。

## 何时不用

- **如果你不使用基于 CLI 的 AI 智能体**——RTK 是命令行工具输出的代理。如果你使用 IDE 内置智能体（Cursor、Copilot）或 Web UI，则没有 shell 输出可供拦截。
- **如果你的智能体已有智能上下文管理**——部分智能体（例如 Claude Code 内置压缩）已经截断和总结。当智能体较原始，或你希望在 shell 层面获得确定性压缩时，RTK 才显价值。
- **如果你需要 100% 输出保真**——RTK 的设计就是压缩和过滤。如果你需要为 LLM 保留每个字节（例如精确二进制 diff、准确字节数），代理可能会丢弃信息。
- **如果你在使用单一二进制不支持的平台**——RTK 为常见平台提供预编译二进制，但小众架构可能需要从源码编译。
- **如果你的工作流不重度依赖 shell**——如果你主要通过文件编辑和自然语言与 AI 智能体交互，很少执行命令，RTK 的节省将很有限。
