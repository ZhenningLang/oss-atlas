...
|---|---|---|---|
| Cline | 未收录 | Choose Cline when you want the leaner open-source VS Code coding-agent upstream. | Open-source VS Code coding agent; part of Kilo Code's lineage [推断]. Similar in-IDE agent loop; Kilo layers on modes/orchestrator and its own model marketplace. Compare directly if you want the leaner upstream. |
...
| [oh-my-claudecode](oh-my-claudecode.md) | ✅ | Choose oh-my-claudecode when you need an orchestration layer on top of Anthropic's Claude Code CLI. | Orchestration *layer on top of* Anthropic's Claude Code CLI (team pipelines, model routing, tmux). Kilo Code is a standalone in-IDE agent, not a wrapper around another agent CLI. |
