...
| [DSPy](dspy.zh.md) | ✅ | 需要模型无关、库形态的程序化 prompt / pipeline 优化框架时，选 DSPy。 | 程序化 prompt / pipeline 优化框架，模型无关、库形态；你写 Python 而非斜杠命令。层次完全不同——是编译，而非 harness 内的评审小组。 |
| [AgentScope](agentscope.zh.md) | ✅ | 需要通用多 agent 运行时/库来构建应用时，选 AgentScope。 | 通用多 agent 运行时 / 库，你在其上构建应用；不绑 Claude Code，也不对“盲点共识”持立场。 |
| [Symphony](symphony.zh.md) | ✅ | 需要由跟踪器驱动的 coding-agent 运行编排时，选 Symphony。 | OpenAI 的多 agent 编排；锚定在 OpenAI 而非 Claude，是框架不是插件。 |
| [openfang](openfang.zh.md) | ✅ | 需要编排模型不同的同类 agent 框架时，选 openfang。 | 同类 agent 框架，编排模型不同；选型前先比范围。 |
...
- **响应速度**：Grade A——中位首次响应时间 0.4 小时，基于 50 个 qualifying issues/PRs。
