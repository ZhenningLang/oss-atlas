...
| [claude-octopus](claude-octopus.zh.md) | ✅ | 需要并行运行多个 Claude Code agent 时，选 claude-octopus。 | 并行编排多个 Claude Code agent；Symphony 以 Codex 为中心、由跟踪器（Linear）驱动，所以选择往往取决于你已经标准化了哪个 agent CLI。 |
...
| [DSPy](dspy.zh.md) | ✅ | 需要编程/优化 LLM 流水线，而不是调度整次实现运行时，选 DSPy。 | 编排/优化 LLM 流水线；是正交问题——DSPy 构建 agent 的推理，Symphony 调度并隔离整次实现运行。 |
