...
你是一支企业团队，正在构建一个大型、复杂的 Web 应用，包含数十个页面、严格的编码规范，并对长期可维护性有要求。你的团队看重强类型、开箱即用的框架和清晰的架构模式（依赖注入、模块、服务）。你选择 Angular，因为它自带你所需的一切：功能强大的 CLI 脚手架、响应式表单系统、HTTP 客户端、支持懒加载的路由器以及一流的 TypeScript 体验。你不想花数周时间评估和拼接第三方库来解决路由、状态管理或表单验证。Angular 的 opinionated 结构意味着新员工能更快上手，因为代码库遵循可预测的模式。

## 何时不用

- **小型或简单项目**——对于落地页、博客或不足 10 个屏幕的 CRUD 后台，Angular 的样板代码和构建复杂度是大材小用。Vite + React 或 Vue 能更快交付。
- **抗拒 TypeScript 的团队**——Angular 深度原生依赖 TypeScript。如果你的团队偏好纯 JavaScript，或觉得 TS 装饰器和复杂类型是负担，摩擦将持续存在。
- **快速原型验证**——Angular 严格的模块系统、构建流程和样板代码会拖慢 MVP 的产出。对于黑客马拉松和原型，更轻的框架（或不用框架）更合适。
- **SEO 优先的静态站点**——虽然 Angular 支持 SSR（Angular Universal），但在静态站点生成方面不如 Next.js 或 Nuxt  seamless。如果你的内容以静态为主且对 SEO 至关重要，请另寻他处。
- **异构微前端**——Angular 的 zone.js 和 Ivy 编译器在与非 Angular 微前端混合时会产生集成摩擦。如果你的架构需要混合框架的 shell，复杂度是真实存在的。[推断]
- **对包体积敏感的场景**——Angular 核心框架比 React 或 Vue 更大。面向低带宽或移动优先的新兴市场的应用，初始加载体积可能是个问题。
...
- **维护**：非常活跃——由 Google 维护，6 个月一个主版本周期，有公开路线图。最近两个主版本提供长期支持（LTS）。[推断]
- **治理**：Google 所有。Angular 团队历史上不受 Google 内部重组影响，但仍是单厂商项目。社区通过 Angular Community Discord 和 GitHub 有发言权。[推断]
- **背书**：Google 是主要背书方。Angular 在 Google 内部被使用（Google Cloud Console、Firebase Console 等），这提供了持续维护的强激励。[推断]
- **采用度**：企业采用度强，100.4k stars，2014 年创建（12 年记录）。是大型企业和金融科技代码库的中流砥柱。就业市场健康，尤其在企业咨询领域。[推断]
- **风险旗标**：MIT 许可宽松。Google 维护 Angular 的记录良好，但「Google 会砍掉项目」的风险始终是背景噪音。未见 relicense 历史。从 zone.js 到 Signals 的架构转变是重大变化；现有应用可能需要迁移。[推断]
