...
| LiteLLM / portkey-style LLM proxy | 未收录 | Choose a purpose-built LLM proxy when all traffic is provider routing and model policy; choose Kong when LLM routing must share one gateway with broader HTTP/API policy. | Purpose-built LLM routers focused only on multi-provider LLM traffic; narrower than Kong's combined HTTP + AI gateway, but lighter if LLM routing is all you need. |
