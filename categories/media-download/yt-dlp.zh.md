...
你在构建媒体管线、归档内容，或只是需要从流媒体站点抓取音视频以便本地处理。你想要一个开箱即支持数百站点的 CLI 工具，能自动选择最佳质量流、合并格式、嵌入字幕、跳过赞助片段，并能作为 cron 任务或内嵌在 Python 脚本中运行。你选择 yt-dlp，因为它是活跃维护的分叉，当站点改变布局或播放器逻辑时，能在数天内修复提取器。一次 pip 安装，一条命令，它就能解析 URL、选择格式、按可预测的文件名模板写入文件。

## 何时不用

- **DRM 保护内容**——yt-dlp 无法解密 Widevine、PlayReady 或 FairPlay DRM；面对受保护流时它会失败或仅返回未加密部分。[未验证]
- **批量商业用途或违反服务条款的使用**——许多站点在服务条款中禁止下载；在未核查法律风险的情况下基于 yt-dlp 构建产品是危险的。youtube-dl 本身曾在 2020 年遭受 DMCA 下架（后恢复）。
- **无提取器的重度 JS 单页应用**——它不执行任意页面 JavaScript；对把媒体藏在重度客户端渲染或按请求令牌方案之后的站点，若无书面提取器就会失败。它不是无头浏览器爬虫。
- **大规模地理限制或登录墙内容**——虽然可以传递 cookie 和代理，但它不会解决验证码、轮换身份或屏蔽 IP 封禁。单一 IP 批量下载会被限速。
- **需要稳定的库 API**——它可以作为 Python 模块导入，但内部 API 和提取器行为会不经通知地改变——适合脚本，作为 shipped 产品的硬依赖则有风险。
- **直播抓取或极高并发**——直播 HLS/DASH 抓取和大规模并行作业很脆弱；专用流媒体工具处理得更好。
...
- [未验证] 支持的站点精确数量（“数千”）随时间变化；请用 `--list-extractors` 核实你的具体目标站点。
- [未验证] SponsorBlock 集成和其他高级功能可能需要额外依赖或默认未启用的配置。
- [推断] 高 star 数（174k）既反映真实实用性，也得益于作为广为人知的 youtube-dl 项目继任者的曝光加成。
- [未验证] 某些区域或 niche 站点的提取器可能由社区贡献，测试深度不如核心 YouTube 提取器。
