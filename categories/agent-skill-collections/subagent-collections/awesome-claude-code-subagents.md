...
| antfu/skills, Dimillian/Skills, gstack, khazix-skills, … | 未收录 / [dimillian-skills](../personal-collections/dimillian-skills.md) ✅ | Choose personal *skill* collections when you need the `Skill`-tool format rather than subagent personas. | Personal *skill* collections (the `Skill`-tool format), not subagent personas. Different unit of consumption — skills are on-demand procedures, subagents are delegated sub-conversations. Use those when you want behaviors loaded into the main agent, this when you want separate delegated experts. |
