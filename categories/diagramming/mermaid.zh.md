...
| 替代品 | 是否收录 | 我们的评价 | 取舍 |
|---|---|---|---|
| Graphviz / DOT | 未收录 | 当前页用于它的主场景；如果更看重“成熟、可脚本化的图**布局**引擎，对大/稠密图有强算法”，再选 Graphviz / DOT。 | 成熟、可脚本化的图**布局**引擎，对大/稠密图有强算法；自动布局极佳，但 DOT 更底层，且不像 Mermaid 那样被文档平台原生内联渲染。 |
| PlantUML | 未收录 | 当前页用于它的主场景；如果更看重“UML 覆盖更广更严（图类型也更多）”，再选 PlantUML。 | UML 覆盖更广更严（图类型也更多）；通常需要 Java 运行时/服务来渲染，而 Mermaid 是纯 JS 浏览器内渲染、宿主支持无处不在。 |
| D2 | 未收录 | 当前页用于它的主场景；如果更看重“较新的文本转图语言（Go），有多种布局引擎（含 ELK/dagre）、追求更干净的排版”，再选 D2。 | 较新的文本转图语言（Go），有多种布局引擎（含 ELK/dagre）、追求更干净的排版；安装量小得多，内建宿主平台渲染远不如 Mermaid。 |
| draw.io(diagrams.net) | 未收录 | 当前页用于它的主场景；如果更看重“完整的所见即所得画布编辑器”，再选 draw.io(diagrams.net)。 | 完整的所见即所得画布编辑器——像素级控制、丰富图形——但图存为 XML/二进制，不是可 diff 的纯文本，也不会从围栏块自动渲染。 |
| Excalidraw | 未收录 | 当前页用于它的主场景；如果更看重“手绘风的所见即所得白板”，再选 Excalidraw。 | 手绘风的所见即所得白板；适合草图和协作，不是文本转图语法，源文件也不能在版本库里 diff。 |
| flowchart.js | 未收录 | 当前页用于它的主场景；如果更看重“只做流程图的窄 JS 库”，再选 flowchart.js。 | 只做流程图的窄 JS 库；Mermaid 覆盖的图类型多得多，生态/宿主支持也大得多。 |
