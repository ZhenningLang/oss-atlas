...
| 替代品 | 是否收录 | 取舍 |
|---|---|---|
| Darts | 未收录 | 现代、维护中的 Python 预测库（含多种模型，包括深度学习），统一 API；面向生产，比一段教学脚本重得多。 |
| GluonTS | 未收录 | 概率时序工具箱（AWS）；做真实的规模化预测很强，学习曲线更陡，不是入门样例。 |
| Prophet | 未收录 | 基于分解的预测，对业务季节性极易上手；不是深度学习/LSTM 的演示。 |
| Keras 官方 RNN 教程 | 未收录 | 同类技术的最新、维护中的 TF2 示例；叙事不如配套文章，但不会以同样方式腐烂。 |
| [PyTorch-GAN](pytorch-gan.zh.md) | ✅ | 领域不同（生成图像），但*体裁*相同——单一作者的参考实现合集，意在教学，而非当作维护中的依赖。 |
